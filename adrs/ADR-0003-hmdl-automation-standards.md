...
7. **Email notification convention**: SMTP configuration across all HMDL modules is standardized on the variable names `mail_smtp_host`, `mail_smtp_port`, `mail_smtp_username`, `mail_smtp_password`, `mail_from`, and `mail_recipients` (list). Email delivery is performed by a dedicated Python script (`roles/*/files/send_notification_email_smtp.py`) invoked via Ansible `command` — not via an inline shell heredoc. Email content (HTML, plain text, JSON data) is written to temporary files first and passed to the script as file-path arguments. This design eliminates `ARG_MAX` failures and heredoc special-character corruption that occur when large host counts cause email bodies to exceed shell limits.
